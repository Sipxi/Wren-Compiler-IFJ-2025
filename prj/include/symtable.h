...
typedef struct{
    ...
    /* Для генератора целевого кода*/
    bool live_status; // Статус "живости" переменной (для генератора целевого кода)
    // ! true если переменная программированнаяя, false если вспомагательная
    // ! для Миши и Вероники True
    // ! для Серегея False
    int next_use; // Последнее использование переменной (для генератора целевого кода)
    // ! По умолчанию для всех -1
