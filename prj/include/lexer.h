 ...
 * @file lexer.h
 * 
 * Заголовочный файл для лексера, отвечающего за токенизацию исходного кода.
 * 
 * Этот файл содержит определения структуры Lexer и прототипы функций
 * для инициализации, освобождения и обработки токенов.
 * Author:
 *     - Serhij Čepil (253038)
...
/**
 * @brief Просматривает следующий токен из исходного кода без его удаления из потока.
 *
 * Эта функция читает символы из предоставленного файла и выдаёт следующий токен
 * 
 * @note Если токен не был прочитан записывается в буфер лексера.
 * 
 * 
 * @param lexer Указатель на структуру Lexer.
...
/**
 * @brief Просматривает следующий за следующим токен из исходного кода без его удаления из потока.
 * 
 * Эта функция читает символы из предоставленного файла и выдаёт токен, следующий за следующим.
 * 
 * @note Если токен не был прочитан записывается в буфер лексера 2 токена.
 * 
 * @param lexer Указатель на структуру Lexer.
 * @param file Указатель на файл, содержащий исходный код.
 * @return Следующая за следующей структура Token.
 */
 ...
 * Если был выполнен предварительный просмотр токена с помощью peek_token,
 * возвращается тот же токен без чтения нового.
 *
 * @param lexer Указатель на структуру Lexer.
 * @param file Указатель на файл, содержащий исходный код.
 * @return Следующая структура Token.
